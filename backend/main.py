...
from models import (
    Base, ResearchSession, ProviderReport, MasterReport,
    ResearchRequest, ResearchSessionResponse, ProviderReportResponse,
    MasterReportResponse, ResearchProgress, MergeRequest,
    ResearchStatus, Provider, WebSocketMessage
)
...
async def export_to_pdf(
    ...
    result = await db.execute(
        select(MasterReport)
        .where(MasterReport.session_id == session_id)
    )
