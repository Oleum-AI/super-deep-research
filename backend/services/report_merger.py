...
class ReportMerger:
    ...
    async def merge_reports(self, reports: List[ProviderReport]) -> MasterReportResult:
        ...
        master_report_result = await self.generate_master_report(
            merged_sections, reports
        )
        ...
        for line in lines:
            ...
            if header_match:
                ...
                current_section = Section(
                    title=title, content="", level=level, provider=provider
                )
                current_content = []
            else:
    ...
    async def merge_section_groups(
        self, grouped_sections: Dict[str, List[Section]]
    ) -> List[Dict]:
...
"""
        ...
        try:
            ...
            result = await provider.generate_research_report(
                topic=merge_prompt, max_output_tokens=2000, include_web_search=False
            )
    ...
    async def generate_master_report(
        self, merged_sections: List[Dict], original_reports: List[ProviderReport]
    ) -> MasterReportResult:
...
"""
    ...
    def create_fallback_report(
        ...
        for section in merged_sections:
            ...
            report += (
                f"*Sources: {', '.join([p.value for p in section['sources']])}*\n\n"
            )
